...
void GoGoPowerRangers(layer &l, double desired_density, const unsigned int direction_flag = 0) {
	...
	static int strat_quad = 0, strat_quad_2 = 0;
	...
	while (cur_density < desired_density) {
		...
		for (int i = 0; i < 100; i++) {
			...
			if (direction) {
				line.genHorizontal();
//				line.y *= (0.01 * ((strat_quad % 100) + 1));
//				strat_quad++; strat_quad %= 100;
			} else {
				line.genVertical();
//				line.x *= (0.01 * ((strat_quad_2% 100) + 1));
//				strat_quad_2++; strat_quad_2 %= 100;
			}
...
/*			switch(direction_flag) {
				case 0:
					line.genHorizontal();
					direction = 0;
					line.y *= (0.25 * (strat_quad) % 4);
					strat_quad++; strat_quad %= 4;
					break;
				case 1:
					line.genVertical();
					line.x *= (0.25 * (strat_quad) % 4);
					strat_quad++; strat_quad %= 4;
					direction = 1;
					break;
				case 2:
			...
			double test_fitness = (test_density - cur_best) * line.length;
